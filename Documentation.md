...
3. **Build with CMake:** Execute the build command. This command invokes the native build tool (like MSVC or Make) to compile the C++ source code and link it into a Python extension module.14 The  
   ...
   The build system is configured to place the final module file (e.g., mlscript.pyd on Windows, mlscript.so on Linux/macOS) in the project's root directory, making it immediately accessible to Python scripts in that location.1

### **Running mlscript**

Because mlscript is a Python module, you interact with it via Python host scripts. The project includes scripts for an interactive session and for executing files.

#### **The Interactive REPL (Read-Eval-Print Loop)**

The REPL is ideal for quick experiments and learning the language syntax. To start it, run repl.py from the project's root directory.
...
For larger programs, you can save your code in a file with a .ms extension and execute it using the run.py script.
...
To run this script, pass its path to run.py:

Shell

python run.py hello.ms
...
let x \= 10 // This part of the line is also ignored.  
print(x) // Prints 10
...
Variables are used to store data. They must be declared before use and are defined within a specific scope.
...
After a variable has been declared with let, its value can be updated by assigning a new value to it without the let keyword.
...
let global\_var \= "I am global"

{  
  let inner\_var \= "I am local"  
...
These statements execute different blocks of code based on a condition.
...
A for loop is used for iterating over a sequence. In v0.2, the only available sequence generator is range(\<start\>, \<end\>), which creates a sequence of integers starting from \<start\> and ending just before \<end\>.
...
Functions are reusable blocks of code that perform a specific task. They help organize code into logical, modular units.
...
fun calculate\_stats() {  
  ...
  let count \= 0  
  let sum \= 0.0  
  let min\_val \= 1000.0 // Start with a very high number  
  let max\_val \= \-1000.0 // Start with a very low number

  // The 'for' loop iterates from 0 to 5 to process our 6 data points.  
  // This is the core of our data processing logic.  
  for i in range(0, 6\) {  
    let current\_val \= 0.0
  ...
  // This demonstrates using the 'return' values from our calculations.  
